...
bq query --project_id=$projectname --destination_table=$desttable --allow_large_results "SELECT url, count(url) as cnt FROM [bigquery-public-data:hacker_news.full] GROUP BY url ORDER BY cnt DESC;"

directoryquery=$(sed -e "s/\${1}/$querytable/" sql/hackernews-directories.sql)
...
filenamesquery=$(sed -e "s/\${1}/$querytable/" sql/hackernews-filenames.sql)
...
jq -r '.[]["url"]' < output/json/${datasetname}.urls_directories.json > output/compiled/${datasetname}.urls_directories.txt
...
jq -r '.[]["url"]' < output/json/${datasetname}.urls_filenames.json > output/compiled/${datasetname}.urls_filenames.txt
